  ...
  try {
    if (req.cookies['connect.sid'] && req.user) {
      ...
      res.status(200).json({
        ...
        topTenHoldings: data.holdings
          .filter(holding => holding.type === 'topHoldings')
          .map(holding => holding.name),
        sectorHoldings: data.holdings
          .filter(holding => holding.type === 'sector')
          .map(holding => {
            return {
              name: holding.name,
        ...
        countryHoldings: data.holdings
          .filter(holding => holding.type === 'country')
          .map(holding => {
            return {
              name: holding.name,
              weight: String(holding.weights.weight) + '%'
